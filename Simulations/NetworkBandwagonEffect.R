...
T <- 100
prop_with_trait <- matrix(nrow=T,ncol=1)
...
for (t in 1:T) {
  ...
  for (i in 1:N) {
    ...
    # adopt the trait with probability p * prop_neighbors_with_trait
    if (trait[j] == 1 && trait[i] == 0) {
      trait[i] <- rbinom(1, 1, p * prop_neighbors_with_trait)
    }
  }
  
  # Calculate the proportion of agents with the trait at the end of the simulation
  prop_with_trait[t,1] <- mean(trait)
